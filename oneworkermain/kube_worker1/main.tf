...
resource "vsphere_virtual_machine" "vm" {
  name             = "kube-2"
  resource_pool_id = data.vsphere_resource_pool.pool.id //changement
 ...
 connection {
  ...
  provisioner "remote-exec" {
   inline = [
    "sleep 10",
	  "sudo sed -i 's/#DNS=/DNS=10.5.1.105/g' /etc/systemd/resolved.conf",
      "sleep 1",
      "sudo sed -i 's/#Domains=/Domains=cloud-temple.lan/g' /etc/systemd/resolved.conf",
      "sleep 1",
